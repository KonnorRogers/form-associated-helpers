...
`;var lt=class extends R{constructor(){super(),this.placeholder="",this.hideBreadcrumbs=!1,this.breadcrumbHome="Home",this.expanded=!1,this.controls="",this.searchLabel="",this.activeDescendant="",this.breadcrumbs=[],this._inputRef=$t()}render(){let t="";if(!this.hideBreadcrumbs){let e=[];if(this.breadcrumbs)for(let l of this.breadcrumbs)e.push(N`<button
              type="button"
              @click=${()=>this.selectParent(l)}
              class="breadcrumb"
            >
              ${l}
            </button>`);t=N`<div class="breadcrumb-list" id="breadcrumb-list">
        <button
          type="button"
    ...
    `}renderAsLink(t){let e=this.action.attributes||{},{download:l,hreflang:o,ping:i,referrerpolicy:r,rel:a,target:c,type:n}=e;return N`
      <a
        class="ninja-action"
        part="ninja-action ${this.selected?"ninja-selected":""}"
        class=${bt(t)}
    ...
    `}escapeString(t){let e=this.constructor;return t.replaceAll(/&/g,e.escapeMap["&"]).replaceAll(/[<>'"]/g,l=>e.escapeMap[l]||l)}encodeString(t){let e=this.constructor;return t.replaceAll(/&/g,`<span>${e.escapeMap["&"]}</span>`).replaceAll(/[<>'"]/g,l=>`<span>${e.escapeMap[l]||l}</span>`)}renderMatch(t,e){let l=this.escapeString(e),o=this.escapeString(t);return this.searchType==="regex"?Ms(o,l,this.__regexMatchRender):this.searchType==="fuzzy"?ms(o,l,this.__fuzzyMatchRender):""}renderBody(){let t;this.action.mdIcon?t=N`<mwc-icon part="ninja-icon" class="ninja-icon">${this.action.mdIcon}</mwc-icon>`:this.action.icon&&(t=ee(`
        <div class="ninja-icon" part="ninja-icon">
          ${this.action.icon}
        </div>
      `));let{title:e,content:l}=this.action;return e&&this.highlightMatches&&(e=this.renderMatch(this.searchQuery,e)),l&&this.highlightMatches&&(l=this.renderMatch(this.searchQuery,l)),N`
        ...
        ${ps(l,()=>N`<small part="ninja-action__content" class="ninja-action__content">${ee(l)}</small>`)}
    `}renderHotkey(){let t;return this.action.hotkey&&(this.hotKeysJoinedView?t=this.action.hotkey.split(",").map(e=>{let l=e.split("+"),o=N`${hs(l.map(i=>N`<kbd part="ninja-hotkey">${i}</kbd>`),"+")}`;return N`<div part="ninja-hotkeys" class="ninja-hotkey ninja-hotkeys">
            ${o}
          </div>`}):t=this.action.hotkey.split(",").map(e=>{let o=e.split("+").map(i=>N`<kbd part="ninja-hotkey" class="ninja-hotkey">${i}</kbd>`);return N`<kbd part="ninja-hotkeys" class="ninja-hotkeys">${o}</kbd>`})),t}};U(J,"baseName","ninja-action"),U(J,"styles",[ut,S`
      :host {
        display: flex;
        width: 100%;
        padding: 0 4px;
    ...
    `]),U(J,"escapeMap",{"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}),U(J,"properties",{action:{type:Object},selected:{type:Boolean},hotKeysJoinedView:{type:Boolean},content:{},searchType:{attribute:"search-type"},highlightMatches:{type:Boolean,attribute:"highlight-matches"},searchQuery:{state:!0,attribute:!1}});var Ns=N` <div part="modal-footer" class="modal-footer" slot="footer">
  <span class="help">
    <svg
      version="1.0"
  ...
  <span class="help">
    ...
    move to parent
            ...
            .action=${c}
          ></ninja-action>`)}`,r=[];return l.forEach((a,c)=>{let n=c?N`<div class="group-header">${c}</div>`:void 0;r.push(N`${n}${i(a)}`)}),N`
...
lit-html/directives/if-defined.js:
   ...
   *)
*/
//# sourceMappingURL=/form-associated-helpers/bridgetown/static/javascript/defer.VJ7JBS7Y.js.map
