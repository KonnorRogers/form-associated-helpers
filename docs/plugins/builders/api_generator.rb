...
# A builder for API routes, not for components necessarily.
class Builders::ApiGenerator < SiteBuilder
  ...
  def build
    generator do
      custom_elements_file = File.expand_path("../../../custom-elements.json", __dir__)

      next unless File.exist?(custom_elements_file)

      custom_elements_manifest_path = File.read(custom_elements_file)
